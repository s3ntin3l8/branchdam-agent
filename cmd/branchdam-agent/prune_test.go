...
// needs, following openTestQueueStore's t.TempDir()-per-test convention.
...
func TestRunPrunePassDeletesVerifiedAgedFile(t *testing.T) {
	...
	stats, err := runPrunePass(context.Background(), &buf, client, store, cfg, time.Now(), false)
	if err != nil {
...
func TestRunPrunePassSkipsNotYetDone(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassSkipsTooYoung(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassSkipsAlreadyGone(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassSkipsFileChangedSinceQueued(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassRefusesFileOutsideLocalEditRoot(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassDryRunDoesNotDelete(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
	...
	var buf bytes.Buffer
	stats, err := runPrunePass(context.Background(), &buf, client, store, cfg, time.Now(), true /* dryRun */)
	if err != nil {
		t.Fatalf("runPrunePass: %v", err)
	}
	if stats.Pruned != 1 {
...
func TestRunPrunePassSkipsSidecarNeverAskedServer(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
...
func TestRunPrunePassNotFoundOrUnverifiedNeverPruned(t *testing.T) {
	cfg, store, root := pruneTestFixture(t)
	cfg.Prune = config.PruneConfig{Enabled: true, MinAgeHours: 24}
