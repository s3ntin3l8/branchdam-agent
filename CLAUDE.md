...
| `cmd/branchdam-agent/selfupdateagent.go` | `selfUpdateAgent`: implements `tray.SelfUpdater` over a configured `internal/selfupdate.Updater` -- the initial check plus a periodic re-check (`selfUpdate.checkIntervalHours`), stopping permanently once a check reports `ErrVersionNotSemver` -- plus `RollbackAvailable`/`Rollback` (issue #33), deliberately **not** gated on `selfUpdate.enabled` since Rollback makes no network call (see those methods' own doc comments) |
...
| `cmd/branchdam-agent/update.go` | `update -config <path> [-check] [-yes] [-rollback]`: the headless equivalent of the tray's "Install and restart" menu item, gated on `selfUpdate.enabled` exactly like the tray's background check. `-rollback` (`runUpdateRollbackCmd`, issue #33) is a separate, config-free code path -- it never reaches `config.ResolvePath` or the `selfUpdate.enabled` gate, matching Rollback's own no-network contract |
...
- **Each target `Apply` touches is applied through its OWN `go-selfupdate` `Updater` instance,
  never the package's single shared one.** `su.Config.OldSavePath` is a single fixed string per
  `Updater`; reusing one shared `Updater` across a Windows install's two targets would make the
  primary exe's `UpdateTo` call silently overwrite the sibling's just-saved `.previous` backup at
  the same path. A fresh per-target `su.Updater` (no network cost -- just a struct) is what makes
  issue #33's rollback correct. This does NOT fix the pre-existing double download on Windows --
  each target's `UpdateTo` still downloads and checksum-validates independently. See
  `docs/platform-support.md`'s Self-update section for why a genuine single-download fix is
  rejected (it would mean reimplementing go-selfupdate's unexported download/validate pipeline by
  hand, moving this repo's only integrity control out of the library's hands).
- **Rollback (issue #33) restores from a `<target>.previous` backup each `Apply` call leaves
  behind, one `.previous.version` sidecar per `InstallLayout` (not per target, since one `Apply`
  brings every target to the same new version atomically), and is applied in the SAME order
  `Apply` uses -- siblings first, the running exe last -- for the identical safety reason.**
  `internal/selfupdate.Rollback` uses go-selfupdate's exported `update.Apply` in the reverse
  direction (restoring `.previous` onto the target) rather than any unexported machinery. On
  success it removes both the `.previous` backups and the version sidecar, so the tray's "Roll
  back to vX" menu item (and `update -rollback`) disappear again until the next `Apply` creates a
  fresh one -- `HasRollback`/`PreviousVersion` are what the tray menu and CLI poll to decide
  whether to offer the affordance at all. Deliberately **not** gated by `selfUpdate.enabled`:
  Rollback makes no network call, so disabling self-update checking must not also hide an
  operator's ability to undo an update they already applied while it was enabled.
...
- **The tray relaunches after a successful self-update via spawn-then-exit, from
  ...
  already been renamed aside to `<exe>.previous` (issue #33's rollback backup, set via `Apply`'s
  per-target `OldSavePath` -- no longer go-selfupdate's own hidden default `.old` naming) by the
  time `Apply` succeeds there, so Windows needs the spawn path regardless -- using `syscall.Exec`
  on darwin/linux would only add a
