...
| `internal/selfupdate/` | Thin wrapper over `github.com/creativeprojects/go-selfupdate` v1.6.0's `DetectLatest`/`UpdateCommand`, gated by `selfUpdate.enabled` (off by default) -- confined to this one package deliberately, since go-selfupdate's GitHub source pulls in `go-github`/`oauth2`, a meaningfully larger dependency-review surface than the rest of this repo |
