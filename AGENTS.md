...
14. **Detector lifecycle (#78)**: `Runner.ReconfigureDetector` cancels the in-flight `Detector.Watch` via a stored `detectorCancel` + `detectorDone` (guarded by `detectorMu`) and starts a new goroutine when `cardRoots` changes; an empty `roots` slice early-returns without starting a new one (`tray.go:1297-1299`), which is the stop-the-detector path. Replaces the "process restart" path. (`internal/tray/tray.go:1262-1305`)
15. **Tray pause gates (#83 + #84)**: Two independent gates guard the network side. `paused` (atomic.Bool, **session-only — never persisted**) short-circuits `TriggerIngest` / `TriggerDrain` / `TriggerPrune` and drops detector events, but never cancels an in-flight ingest. `pauseUploadOnMetered` (a persisted `ingest.pauseUploadOnMetered` config key, edited via the Settings menu, **survives restarts**) + `netgate.IsMetered()` true short-circuits `TriggerDrain` only — local edit copy proceeds regardless. The drain gate's `metered || mErr != nil` short-circuit (tray.go:826) is the fail-closed guarantee at the call site; `netgate.IsMetered` itself is best-effort (e.g. `netgate_linux.go`'s `checkSysfsWWAN` returns `(false, nil)` on its own `ReadDir` failure). (`internal/tray/tray.go:241-257,766-829`; `internal/netgate/`)
...
17. **M5 tray UX wiring (#79 + #80 + #81 + #86 + #87)**: Five tray behaviors wired through Settings submenus and the headless `ingest` subcommand's `-allowed-extensions` override: (a) `IngestGate` confirmation dialog wrapping the detector's `triggerIngest(..., isDetection=true)` (tray.go:478-502) with an `autoImportPaths` allow-list; manual `TriggerIngest` (incl. "Import from folder…", run_supported.go:385) bypasses it. A dialog-render error is NOT added to the skip set (only explicit refusals are). (b) "Import from folder…" manual source picker reusing the same `dialogRunner`. (c) `RequireDCIM` (detection-only filter at `ListVolumesUnder`/`NewDetector`) + `AllowedExtensions` (walk-time filter at `shouldSkipByExtension`, ingest.go:235 / offline.go:128); empty `allowedExtensions` = "accept all" (backward-compat). (d) `PathTemplate` overwritten by `Handshake.NamingTemplate` at tray startup and on every `reload()` — operator edits survive only between Handshakes. (e) `autoEject` only fires when the ingest summary is `OK()`, via the platform-tagged `internal/eject` package (Linux: `udisksctl unmount -b <dev>` then `udisksctl power-off -b <dev>`; Darwin: `diskutil unmountDisk`; Windows: `IOCTL_STORAGE_EJECT_MEDIA` via `DeviceIoControl`; other: `ErrUnsupported` stub).
