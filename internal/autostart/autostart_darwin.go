...
// The plist uses -read-args with a JSON sidecar to avoid embedding
// potentially dangerous characters (e.g. ;, &, >) in the command.
func Enable(execPath string, args []string) error {
	...
	if err := os.WriteFile(plistPath, []byte(RenderLaunchAgentPlistReadArgs(execPath, sidecar)), 0o644); err != nil {
...
// either way) and removes the plist file and the args sidecar.
func Disable() error {
	home, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("autostart: resolve home directory: %w", err)
